...
To properly configure WALL, update the following settings in `app.py` by replacing placeholder values with actual values.  
   ...
   ```
   ### 3.3.6. Set Up Revised File Path 
   To correctly locate the revised versions of files generated by the LLM, update **Line 450** in `app.py` with the following code:
   ...
   **On macOS/Linux:**  
   ...
   This configuration ensures that the application correctly accesses the revised file in the `WALL.Revised` directory while preserving the original directory structure.

   ### 3.3.7. Set Up OpenAI API Key
...
This page allows users to upload the CSV file generated from the **Issue Extractor Tool** and interactively revise the code issues.

**Steps**:
   - **Upload CSV File**: Users upload the CSV file generated in the **Issue Extractor Tool** section.
   - **Select Issues**: From the uploaded CSV file, users can browse and select individual files with issues.
   - **View & Edit**:
   ...
   - **Choose GPT Model**: Select the desired GPT model (e.g., GPT-3.5 Turbo or GPT-4o) to process the prompt.
   - **Send Prompt**: Send the customized or preset prompt to the OpenAI API for code revision.
   - **View & Save Revised Code**:
      - Review the revised version of the file generated by the AI.
      - Save the revised file to a preferred location.
   - **Prepare for Code Comparison** *(Optional)*: If users intend to use the **Code Compare Tool**, they need to organize the revised files in a specific format:
